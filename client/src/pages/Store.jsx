...
function Store() {
  ...
  const [timerModalOpen, setTimerModalOpen] = useState(false); // modal del juego al completar compra
  ...
  // En modo edición siempre se muestra la vista normal del tótem (nunca el mapa de mesas)
  ...
  // La lluvia de emojis ahora es liviana (solo 6–9 glyphs, animación barata sin
  // filtros), así que SÍ se muestra en la mayoría de equipos. Solo la ocultamos en
  // gama MUY baja (≤2 núcleos o ≤2GB) o si el sistema pide menos animación.
  ...
  const handleCheckout = async () => {
    ...
    // Juego del cronómetro: se muestra un modal grande antes de pagar. Cuando
    // el cliente termina su intento (gane o pierda), pulsa "Continuar" y se
    // vuelve a llamar a handleCheckout (ya con timerPlayed=true) para pagar.
    if (timerGameConfig?.enabled && !timerPlayed) {
      resetTimerGame();
      setCartOpen(false);
