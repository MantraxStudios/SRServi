...
function WorkerPanel() {
  ...
  useEffect(() => {
    ...
    if (parsedWorker.store_code) {
      const BASE = 'https://srservi3.srautomatic.com';
  ...
  const downloadPrepTablePDF = (table) => {
    ...
    const bodyRows = Array.from({ length: maxRows }, (_, rowIdx) => {
      const tds = cols.map(col => {
        ...
        const imgUrl = cell.image_url ? (cell.image_url.startsWith('http') ? cell.image_url : 'https://srservi3.srautomatic.com' + cell.image_url) : null;
        let content = '';
      ...
      <div className="worker-orders" style={activeTab === 'tasks' || activeTab === 'mesas' || activeTab === 'ventarapida' || activeTab === 'rankings' || activeTab === 'comments' ? { padding: 0 } : undefined}>
                ...
                {customCreations.map(c => (
                  <button key={c.id} onClick={() => setActiveCustomCreation(c)}
                    ...
                    <div style={{ width: 100, flexShrink: 0, background: '#1a1a1a', position: 'relative', minHeight: 58 }}>
                      {c.background_image ? (
                        <img src={c.background_image.startsWith('http') ? c.background_image : 'https://srservi3.srautomatic.com' + c.background_image} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                      ) : (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {procedures.map(proc => {
                  ...
                  const imgUrl = firstStep?.image_url
                    ? (firstStep.image_url.startsWith('http') ? firstStep.image_url : 'https://srservi3.srautomatic.com' + firstStep.image_url)
                    : null;
                  return (
                    <button
                      key={proc.id}
        ...
        {activeTab === 'procedures' && selectedProc && (() => {
          ...
          const imgUrl = step?.image_url
            ? (step.image_url.startsWith('http') ? step.image_url : 'https://srservi3.srautomatic.com' + step.image_url)
            : null;
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', padding: 8 }}>
              <div style={{ position: 'relative', width: '100%', maxWidth: '100%', aspectRatio: `${CW}/${CH}` }}>
                <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', borderRadius: 8 }}>
                  {/* Fondo */}
                  {c.background_image ? (
                    <img src={c.background_image.startsWith('http') ? c.background_image : 'https://srservi3.srautomatic.com' + c.background_image}
                  ...
                  {(c.elements || []).map(el => el.type === 'image' ? (
                    <div key={el.id} style={{
                      ...
                      <img src={el.imgUrl.startsWith('http') ? el.imgUrl : 'https://srservi3.srautomatic.com' + el.imgUrl} alt=""
