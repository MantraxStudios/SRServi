...
export default function AiImageGenerator() {
  ...
  const generate = async () => {
    if (!storeId) return;
    if (!prompt.trim()) { showToast('Escribí una descripción de la imagen primero', 'error'); return; }
    setGenerating(true);
    try {
      ...
      if (!res.ok) throw new Error((await res.json().catch(() => ({}))).error || 'Error generando la imagen');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      setCurrent(url);
      setGallery(g => [{ url, prompt: prompt.trim(), ts: Date.now() }, ...g].slice(0, 12));
    } catch (e) { showToast(e.message, 'error'); }
    finally { setGenerating(false); }
  };

  const download = (url, idx) => {
    const a = document.createElement('a');
    a.href = url;
    a.download = `ia-${selectedStore?.code || 'imagen'}-${idx ?? Date.now()}.png`;
    a.click();
  };
    ...
    <div className="aig-page">
      ...
      <div className="aig-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={s.card}>
            <h3 style={s.cardTitle}>Describí la imagen</h3>
            <div style={s.field}>
              <textarea
                value={prompt}
                onChange={e => setPrompt(e.target.value)}
                rows={4}
                placeholder="Ej: hamburguesa gourmet con papas fritas, fondo colorido, foto publicitaria profesional"
                style={{ ...s.input, resize: 'vertical', height: 'auto' }}
              />
              <p style={s.hint}>Sé específico: producto, ambiente, estilo. Cuanto más detalle, mejor el resultado.</p>
            </div>
            ...
            {showAdvanced && (
              <div style={s.field}>
                <label style={s.label}>Evitar en la imagen <span style={{ color: '#9ca3af', fontWeight: 400 }}>(opcional)</span></label>
                <input
            ...
            <button onClick={generate} disabled={generating} style={{ ...s.btnPrimary, opacity: generating ? 0.7 : 1 }}>
              {generating ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faMagicWandSparkles} />}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={s.card}>
            ...
            {current ? (
              <>
                <img src={current} alt="Imagen generada" style={{ width: '100%', display: 'block', borderRadius: 10, marginBottom: 10 }} />
              ...
              <div style={{ background: '#f8fafc', padding: '40px 20px', textAlign: 'center', borderRadius: 10 }}>
                <FontAwesomeIcon icon={faImage} style={{ fontSize: 32, color: '#cbd5e1', marginBottom: 10 }} />
                <p style={{ color: '#94a3b8', fontSize: 13, margin: 0 }}>
                  Describí una imagen a la izquierda y hacé clic en "Generar imagen"
                </p>
              </div>
            )}
          </div>
                ...
                {gallery.map(item => (
                  <div key={item.ts} style={{ position: 'relative', borderRadius: 10, overflow: 'hidden', border: '1px solid #e5e7eb' }}>
                    ...
                    <div style={{ position: 'absolute', top: 6, right: 6, display: 'flex', gap: 4 }}>
                      <button onClick={() => download(item.url, item.ts)} style={s.galleryBtn}>
