      ...
      if (paymentRes.ok) {
        ...
        if (payment.status === 'approved' && payment.external_reference) {
          ...
          if (orders.length > 0) {
            ...
            notifyOrderStatusWhatsApp(store.id, orders[0].id, 'preparing');
          }
        }
      }
    }
...
app.post('/api/store-configurations', authenticateToken, async (req, res) => {
  try {
    const { store_id, name, description, accept_cash, accept_card, is_active, is_default, is_minimarket, default_minimarket_terminal, allow_serve, allow_takeout, hide_decimals, allow_table_service, tip_percentage, delivery_enabled, delivery_payment_methods, require_order_comment, require_customer_name } = req.body;
    if (!store_id) {
      return res.status(400).json({ error: 'store_id es requerido' });
    }
    const isOwner = await verifyStoreOwnership(parseInt(store_id), req.user.id);
...
app.put('/api/public/:code/store-configurations/:id', async (req, res) => {
  try {
    const auth = await verifyStoreAccess(req.params.code, req.body);
    if (!auth.authorized) return res.status(auth.status || 403).json({ error: auth.error });
    const { name, description, accept_cash, accept_card, is_active, is_default, is_minimarket, default_minimarket_terminal, allow_serve, allow_takeout, hide_decimals, allow_table_service, tip_percentage, delivery_enabled, delivery_payment_methods, require_order_comment, require_customer_name } = req.body;
    if (!name) return res.status(400).json({ error: 'Nombre es requerido' });
    const configuration = await updateStoreConfiguration(parseInt(req.params.id), auth.store.id, { name, description, accept_cash, accept_card, is_active, is_default, is_minimarket, default_minimarket_terminal, allow_serve, allow_takeout, hide_decimals, allow_table_service, tip_percentage, delivery_enabled, delivery_payment_methods, require_order_comment, require_customer_name });
    res.json(configuration);
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
// ── Promociones de tienda (banners en el store) ──────────────────
...
async function notifyOrderStatusWhatsApp(storeId, orderId, status) {
  ...
  try {
    ...
    if (!order.customer_phone) {
      console.log(`[WhatsApp:${sid}] Aviso omitido para orden #${orderId}: sin customer_phone (el cliente no dejó teléfono en el tótem).`);
      return;
    }
    ...
    if (sendSid === null && !isJid && store.user_id) {
      try {
        ...
        for (const s of siblings) {
          if (getWhatsAppStatus(s.id).connected) { sendSid = s.id; break; }
        }
...
app.put('/api/orders/:id/status', authenticateToken, async (req, res) => {
  try {
    ...
    notifyOrderStatusWhatsApp(store_id, order.id, status);
...
app.put('/api/orders/:id/approve-cash', authenticateToken, async (req, res) => {
  try {
    ...
    notifyOrderStatusWhatsApp(store_id, parseInt(id), 'preparing');
    res.json(order);
  } catch (error) {
...
app.put('/api/orders/:id/mark-paid', authenticateToken, async (req, res) => {
  try {
    ...
    notifyOrderStatusWhatsApp(store_id, parseInt(id), paidStatus);
    res.json(order);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
async function startServer() {
  try {
              ...
              try {
                ...
                if (sqStatus === 'COMPLETED') {
                  ...
                  if (order_id) {
                    ...
                    notifyOrderStatusWhatsApp(store_id, order_id, 'preparing');
                  }
                  const socketId = userSockets.get(parseInt(store_id));
    ...
    async function haulmerConfirmOrder(tx, reference, authCode) {
      ...
      notifyOrderStatusWhatsApp(tx.store_id, tx.order_id, 'preparing');
    ...
    async function ensureCctvTables() {
      ...
      for (const sql of [
        ...
        'ALTER TABLE cctv_screens ADD COLUMN music_play_all TINYINT(1) DEFAULT 0',
    ...
    // Admin: reproducir TODA la música en secuencia (playlist). null = desactivar.
    app.put('/api/cctv/screens/:id/music-all', authenticateToken, async (req, res) => {
      try {
        await ensureCctvTables();
        const on = req.body.play_all ? 1 : 0;
        await pool.execute(
          'UPDATE cctv_screens SET music_play_all = ?, current_music_id = NULL WHERE id = ? AND user_id = ?',
          [on, req.params.id, req.user.id]
        );
        res.json({ ok: true });
      } catch (e) { res.status(500).json({ error: e.message }); }
      ...
      try {
        ...
        const [rows] = await pool.execute(`
          SELECT s.id, s.user_id, s.device_name, s.group_id,
            s.current_video_id, s.current_music_id, s.current_album_id, s.current_image_id, s.video_play_all, s.music_play_all, s.video_muted, s.display_mode,
        ...
        let playback = {
          ...
          music_play_all: screen.music_play_all,
        ...
        if (screen.group_id) {
          ...
          if (gRows.length) {
            playback = {
              ...
              music_play_all: 0,      // ni playlist de toda la música
        ...
        const config = {
          ...
          music_play_all: !!playback.music_play_all,
        };
        if (playback.music_play_all) {
          // Reproducir TODA la música en secuencia (playlist en loop).
          const [tracks] = await pool.execute(
            'SELECT url, original_name AS name FROM cctv_music WHERE user_id = ? ORDER BY created_at ASC',
            [userId]
          );
          config.musics = tracks;
          if (!config.music_url && tracks.length) { config.music_url = tracks[0].url; config.music_name = tracks[0].name; }
        }
    ...
    app.put('/api/cctv/screens/:id/music', authenticateToken, async (req, res) => {
      try {
        ...
        await pool.execute('UPDATE cctv_screens SET current_music_id = ?, music_play_all = 0 WHERE id = ? AND user_id = ?', [music_id || null, req.params.id, req.user.id]);
        res.json({ ok: true });
      } catch (e) { res.status(500).json({ error: e.message }); }
    });
...
app.post('/api/stores/:id/subdomain', authenticateToken, async (req, res) => {
  try {
    ...
    const reserved = ['www', 'api', 'mail', 'smtp', 'ftp', 'admin', 'srservi2', 'srservi3', 'app', 'static', 'cdn'];
